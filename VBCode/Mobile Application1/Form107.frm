...
Begin VB.Form Form107 
   ...
   Begin MSComctlLib.ProgressBar PB101 
      ...
      Appearance      =   1
   End
   Begin VB.CommandButton CMDSlotTagBrand 
      Caption         =   "พิมพ์ SlotTag ตามยี่ห้อ"
      BeginProperty Font 
         Name            =   "MS Sans Serif"
         Size            =   8.25
         Charset         =   222
...
vQuery = "exec dbo.USP_MB_SearchShelfCode '" & vWHCode & "','" & vZoneCode & "' "
    If OpenDataBase(gConnection, vRecordset, vQuery) <> 0 Then
        ...
        vItemList.SubItems(2) = Trim(vRecordset.Fields("zonecode").Value)
        vRecordset.MoveNext
        Wend
    End If
    vRecordset.Close
...
If Me.Text101.Text <> "" Or Me.Text102.Text <> "" Or Me.Text103.Text <> "" Then
   ...
   ElseIf Me.Text101.Text <> "" And TXTZone.Text <> "" And Me.Text102.Text = "" And Text103.Text = "" Then
       vWHCode = Text101.Text
       vZoneCode = TXTZone.Text
       vQuery = "exec dbo.USP_MB_SearchItemZoneRecProduct '" & vWHCode & "','" & vZoneCode & "' "
   ...
   ElseIf Me.Text101.Text <> "" And Me.Text102.Text <> "" And TXTZone.Text <> "" And Me.Text103.Text = "" Then
       vWHCode = Text101.Text
       vShelfCode = Text102.Text
       vZoneCode = Trim(TXTZone.Text)
       vQuery = "exec dbo.USP_MB_SearchItemCodeZoneRecProduct '" & vWHCode & "'," & vZoneCode & "','" & vShelfCode & "',''' "
   ...
   ElseIf Me.Text101.Text <> "" And Me.Text102.Text <> "" And TXTZone.Text <> "" And Me.Text103.Text <> "" Then
       ...
       vZoneCode = Trim(TXTZone.Text)
       vItemCode = Trim(Text103.Text)
       vQuery = "exec dbo.USP_MB_SearchItemCodeZoneRecProduct '" & vWHCode & "','" & vZoneCode & "','" & vShelfCode & "',''" & vItemCode & "' "
   ...
   End If
       If OpenDataBase(gConnection, vRecordset, vQuery) <> 0 Then
           ...
           vItemList.SubItems(7) = Trim(vRecordset.Fields("zonecode").Value)
           i = i + 1
           Me.PB101.Value = n
           n = n + 1
           vRecordset.MoveNext
...
vQuery = "exec dbo.USP_NP_SelectReportName 515,'IS' "
If OpenDataBase(gConnection, vRecordset, vQuery) <> 0 Then
...
If Me.Text101.Text <> "" And Me.Text102.Text <> "" And Me.ListView102.ListItems.Count > 0 Then
   ...
   If vCountListItem = 0 Then
      Call vPrintCountSheet(vWHCode, vZoneCode, vShelfCode, vItemCode, 0)
   ...
   ElseIf vCountListItem = Me.ListView102.ListItems.Count And vCountListItem > 1 Then
      Call vPrintCountSheet(vWHCode, vZoneCode, vShelfCode, "", 0)
   Else
         For i = 1 To Me.ListView102.ListItems.Count
            If Me.ListView102.ListItems(i).Checked = True Then
               vItemCode = Me.ListView102.ListItems(i).SubItems(2)
               Call vPrintCountSheet(vWHCode, vZoneCode, vShelfCode, vItemCode, i)
            End If
         Next i
   End If
...
Public Sub vPrintCountSheet(WHCode As String, ZoneCode As String, ShelfCode As String, ItemCode As String, LineNumber As Integer)
...
'vPrinterName = Trim("\\it-queuemedia\TM-Mobile")
...
If WHCode <> "" And ZoneCode <> "" And ShelfCode <> "" And ItemCode = "" Then
   'USP_MB_SearchItemCodeZoneRecProduct
   'vQuery = "exec dbo.USP_MB_SearchItemScanRecProduct '" & WHCode & "','" & ShelfCode & "','" & ZoneCode & "' "
   vQuery = "exec dbo.USP_MB_SearchItemCodeZoneRecProduct '" & WHCode & "','" & ZoneCode & "','" & ShelfCode & "','" & ItemCode & "' "
ElseIf WHCode <> "" And ZoneCode <> "" And ShelfCode <> "" And ItemCode <> "" Then
  vQuery = "exec dbo.USP_MB_SearchItemCodeZoneRecProduct '" & WHCode & "','" & ZoneCode & "','" & ShelfCode & "','" & ItemCode & "' "
End If
...
Private Sub CMDSlotTagBrand_Click()
Call vPrintSlotTagBrand
End Sub
...
i = 1
vQuery = "exec dbo.USP_IV_SearchStoreItem 'OFS' "
If OpenDataBase(gConnection, vRecordset, vQuery) <> 0 Then
        vRecordset.MoveFirst
        While Not vRecordset.EOF
...
Me.DTPDate.Value = DateAdd("d", -1, Day(Now) & "/" & Month(Now) & "/" & Year(Now))
...
If Me.ListView103.ListItems.Count > 0 Then
   ...
   Next i
      ...
      If vCheckCount = 1 Then
         ...
         End If
         vRecordset.Close
      ElseIf vCheckCount = 0 Then
         Me.ListView104.ListItems.Clear
         vCheckCountShelf = 0
      Else
         MsgBox "สามารถเลือกพิมพ์ได้ทีละ Row เท่านั้น ", vbCritical, "Send Error Message"
         Me.ListView104.ListItems.Clear
         vCheckCountShelf = 0
      End If
...
ListView101.ListItems.Clear
vQuery = "exec dbo.USP_MB_SearchShelfCodeFilter '" & vWHCode & "','" & vZoneCode & "','" & vShelfCode & "' "
    If OpenDataBase(gConnection, vRecordset, vQuery) <> 0 Then
        vRecordset.MoveFirst
        While Not vRecordset.EOF
        Set vItemList = ListView101.ListItems.Add(, , Trim(vRecordset.Fields("shelfcode").Value))
...
vQuery = "select * from dbo.vw_np_countstockprintslottag"
If OpenDataBase(gConnection, vRecordset, vQuery) <> 0 Then
   vRecordset.MoveFirst
   i = 1
   While Not vRecordset.EOF
...
End If
    ...
    Printer.CurrentX = 0
    Printer.CurrentY = Printer.CurrentY
    Printer.Print Trim(vRecordset.Fields("itemname").Value)
