...
Begin VB.Form Form312 
   ...
   Begin TabDlg.SSTab SSTab1 
      ...
      TabPicture(2)   =   "Form312.frx":CD9B
      Tab(2).ControlEnabled=   0   'False
      Tab(2).Control(0)=   "Command1"
      Tab(2).Control(1)=   "CMD302"
      Tab(2).Control(2)=   "CMD301"
      Tab(2).Control(3)=   "Text309"
      Tab(2).Control(4)=   "Text308"
      Tab(2).Control(5)=   "Text307"
      Tab(2).Control(6)=   "Text306"
      Tab(2).Control(7)=   "Text305"
      Tab(2).Control(8)=   "Text304"
      Tab(2).Control(9)=   "Text303"
      Tab(2).Control(10)=   "Text302"
      Tab(2).Control(11)=   "Text301"
      Tab(2).Control(12)=   "Label27"
      Tab(2).Control(13)=   "Label26"
      Tab(2).Control(14)=   "Label25"
      Tab(2).Control(15)=   "Label24"
      Tab(2).Control(16)=   "Label23"
      Tab(2).Control(17)=   "Label22"
      Tab(2).Control(18)=   "Label21"
      Tab(2).Control(19)=   "Label20"
      Tab(2).Control(20)=   "Label19"
...
Dim vReqDate As Date
Dim vReqTime As String

Dim vGetDate As Date
Dim vGetTime As String
...
If ListView101.ListItems.Count <> 0 Then
    ...
    If Me.Text306.Text = "" Then
    MsgBox "กรุณา เลือกเส้นทางขอจัดส่งเอกสาร", vbCritical, "Send Error Message"
    Me.SSTab1.Tab = 2
    Me.Text306.SetFocus
    Exit Sub
    End If
    
    If Trim(MaskEdBox101.Text) = "__:__" Then
    MsgBox "กรุณา กรอกเวลาที่นัดรับสินค้า", vbCritical, "Send Error Message"
    Me.SSTab1.Tab = 0
    Me.MaskEdBox101.SetFocus
    Exit Sub
    End If
    
    vRouteID = Me.Text306.Text
    
    If vRouteID = 5 Or vRouteID = 7 Or vRouteID = 10 Or vRouteID = 11 Then
            vReqDate = Day(Me.DTPicker102.Value) & "/" & Month(Me.DTPicker102.Value) & "/" & Year(Me.DTPicker102.Value)
            vReqTime = Me.MaskEdBox101.Text
            vGetDate = Day(Now) & "/" & Month(Now) & "/" & Year(Now)
            vAddDate = DateAdd("d", 1, vGetDate)
        
            If vAddDate > vReqDate Then
                MsgBox "เส้นทางดังกล่าว ถูกกำหนดให้วันที่ขอส่งคิวสินค้าต้อง 1 วันข้างหน้าขึ้นไป", vbCritical, "Send Information Message"
                Me.SSTab1.Tab = 0
                Exit Sub
            End If
    End If
    
    If vRouteID = 6 Or vRouteID = 7 Or vRouteID = 10 Or vRouteID = 11 Then
            vReqDate = Day(Me.DTPicker102.Value) & "/" & Month(Me.DTPicker102.Value) & "/" & Year(Me.DTPicker102.Value)
            vReqTime = Me.MaskEdBox101.Text
            vGetDate = Day(Now) & "/" & Month(Now) & "/" & Year(Now)
            vAddDate = DateAdd("d", 2, vGetDate)
        
            If vAddDate > vReqDate Then
                MsgBox "เส้นทางดังกล่าว ถูกกำหนดให้วันที่ขอส่งคิวสินค้าต้อง 2 วันข้างหน้าขึ้นไป", vbCritical, "Send Information Message"
                Me.SSTab1.Tab = 0
                Me.Text306.SetFocus
                Exit Sub
            End If
...
If Trim(MaskEdBox101.Text) = "__:__" Then
MsgBox "กรุณา กรอกเวลาที่ต้องการส่งสินค้า", vbCritical, "Send Error Message"
Me.SSTab1.Tab = 0
Me.MaskEdBox101.SetFocus
Exit Sub
End If
