...
Begin VB.Form FormTaxPurchase 
   ...
   Begin VB.OptionButton Opt107 
      Appearance      =   0  'Flat
      BackColor       =   &H80000005&
      Caption         =   "บันทึกตั้งหนี้จากการซื้อ"
      ForeColor       =   &H80000008&
      Height          =   285
      Left            =   10800
      TabIndex        =   35
   ...
   Begin VB.OptionButton Opt105 
      Appearance      =   0  'Flat
      BackColor       =   &H80000005&
      Caption         =   "หน้าบันทึก จ่ายเงินมัดจำ"
      ForeColor       =   &H80000008&
      Height          =   285
      Left            =   8250
      TabIndex        =   33
   ...
   Begin VB.PictureBox Pic101 
      ...
      Begin VB.CommandButton CMD101 
         Height          =   330
         Left            =   225
         Picture         =   "FormTaxPurchase.frx":9673
         Style           =   1  'Graphical
         TabIndex        =   11
         Top             =   1890
         Width           =   915
...
'--------------------------------------------------------------------
    ...
    ElseIf Opt104.Value = True Then
        ...
        vSaveFrom = 1
        vDocType = 4
    ElseIf Opt105.Value = True Then
        vSource = 1
        vSaveFrom = 0
        vDocType = 3
    ElseIf Opt106.Value = True Then
        vSource = 9
        vSaveFrom = 2
        vDocType = 5
    ElseIf Opt107.Value = True Then
        vSource = 2
        vSaveFrom = 1
        vDocType = 6
    End If
...
If Me.Text101.Text = "" And Me.ListView101.ListItems.Count = 0 Then
MsgBox "ไม่มีข้อมูลของเลขที่เอกสาร ในการบันทึกข้อมูล กรุณาตรวจสอบ", vbCritical, "Send Error Message"
Exit Sub
End If
...
'ปรับข้อมูลเลขที่ใบกำกับภาษีหลังร้าน
    ...
    If vDocType = 1 Then
      vQuery = "select count(docno) as vCount from bi.bcvat.dbo.BCAPOTHERDEBT where docno = '" & vDocNo & "' "
      If OpenDataBaseDev(vDevConnection, vRecordset, vQuery) <> 0 Then
        vCount = vRecordset.Fields("vcount").Value
      End If
      vRecordset.Close
    ElseIf vDocType = 2 Then
      vQuery = "select count(docno) as vCount from bi.bcvat.dbo.BCOTHEREXPENSE where docno = '" & vDocNo & "' "
      If OpenDataBaseDev(vDevConnection, vRecordset, vQuery) <> 0 Then
        vCount = vRecordset.Fields("vcount").Value
      End If
      vRecordset.Close
    ElseIf vDocType = 3 Then
      vQuery = "select count(docno) as vCount from bi.bcvat.dbo.BCSTKREFUND where docno = '" & vDocNo & "' "
      If OpenDataBaseDev(vDevConnection, vRecordset, vQuery) <> 0 Then
        vCount = vRecordset.Fields("vcount").Value
      End If
      vRecordset.Close
    ElseIf vDocType = 4 Then
      vQuery = "select count(docno) as vCount from bi.bcvat.dbo.BCAPINVOICE where docno = '" & vDocNo & "' "
      If OpenDataBaseDev(vDevConnection, vRecordset, vQuery) <> 0 Then
        vCount = vRecordset.Fields("vcount").Value
      End If
      vRecordset.Close
    ElseIf vDocType = 5 Then
      vQuery = "select count(docno) as vCount from bi.bcvat.dbo.BCAPDEPOSIT where docno = '" & vDocNo & "' "
      If OpenDataBaseDev(vDevConnection, vRecordset, vQuery) <> 0 Then
        vCount = vRecordset.Fields("vcount").Value
      End If
      vRecordset.Close
    ElseIf vDocType = 6 Then
      vQuery = "select count(docno) as vCount from bi.bcvat.dbo.BCPAYMENT where docno = '" & vDocNo & "' "
      If OpenDataBaseDev(vDevConnection, vRecordset, vQuery) <> 0 Then
        vCount = vRecordset.Fields("vcount").Value
      End If
      vRecordset.Close
    ElseIf vDocType = 7 Then
      vQuery = "select count(docno) as vCount from bi.bcvat.dbo.BCAPINVOICE where docno = '" & vDocNo & "' "
      If OpenDataBaseDev(vDevConnection, vRecordset, vQuery) <> 0 Then
        vCount = vRecordset.Fields("vcount").Value
      End If
    ...
    vQuery = "delete bi.bcvat.dbo.bcinputtax where docno = '" & vDocNo & "' "
    vDevConnection.Execute vQuery
    


    vQuery = "select count(docno) as vCount from bi.bcvat.dbo.BCSTKREFUND where docno = '" & vDocNo & "' "
    If OpenDataBaseDev(vDevConnection, vRecordset, vQuery) <> 0 Then
      vExistDocNo = vRecordset.Fields("vcount").Value
    End If
    vRecordset.Close
    ...
    For i = 1 To ListView101.ListItems.Count
        ...
        vQuery = "insert into bi.bcvat.dbo.BCInputTax (SaveFrom,DocNo,BookCode,Source,DocDate,TaxDate,TaxDate2," _
    ...
    If vExistDocNo > 0 Then
    vQuery = "update bi.bcvat.dbo.BCSTKRefund set taxno = '" & vTaxNo & "' where docno = '" & vDocNo & "' "
    vDevConnection.Execute vQuery
    End If
...
Call SetListViewColor(ListView101, PicPoint, vbWhite, vbLightBlue)
End Sub
...
Private Sub Opt105_Click()
On Error Resume Next
If Opt105.Value = True Then
    Pic101.Visible = True
    Text101.SetFocus
    DTPicker101 = Now
    DTPicker102 = Now
