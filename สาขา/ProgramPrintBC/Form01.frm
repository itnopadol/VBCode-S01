...
Begin VB.Form Form01 
   ...
   Begin Crystal.CrystalReport CrystalQuotation 
      Left            =   2610
      Top             =   8010
      _ExtentX        =   741
      _ExtentY        =   741
   ...
   End
   Begin Crystal.CrystalReport Crystal001 
      Left            =   405
      Top             =   8055
      _ExtentX        =   741
      _ExtentY        =   741
...
If TXT011.Text <> "" And TXT012.Text <> "" Then
   ...
   If vPrint = 1 Then
    ...
    '-----------------------------------------------------------
    vQuery = "select billstatus ,billtype,isconditionsend  from dbo.bcsaleorder where docno = '" & vDocNo & "' "
    If OpenDataBase(gConnection, vRecordset, vQuery) <> 0 Then
    ...
    If vBillStatus = 0 Then
            ...
            ElseIf TXT012.Text = Trim("พิมพ์ใบสั่งขาย+พิมพ์ใบจัดคิวสินค้า") Then
                Call SaleOrder
                vQuery = "select isconditionsend from dbo.bcsaleorder where docno = '" & vDocNo & "' "
                If OpenDataBase(gConnection, vRecordset, vQuery) <> 0 Then
                    vSend = Trim(vRecordset.Fields("isconditionsend").Value)
                End If
                vRecordset.Close
    ...
    ElseIf vDocTypeID = "RQ" Then
        ...
        If TXT012.Text = Trim("พิมพ์ใบเสนอซื้อสินค้าของจัดซื้อ") Then
            If UCase(vDepartment) = "MC" Or UCase(vDepartment) = "MG" Or UCase(vDepartment) = "IT" Or UCase(vDepartment) = "AV" Then
               Call PrintStockRequestApprove
            Else
            MsgBox "รหัสผู้ใช้งานไม่มีสิทธิ์พิมพ์เอกสารดังกล่าว กรุณาตรวจสอบ", vbCritical, "Send Error Message"
            Exit Sub
            End If
        End If
        
        If TXT012.Text = Trim("พิมพ์ใบขอราคาสินค้าของจัดซื้อ") Then
            If UCase(vDepartment) = "MC" Or UCase(vDepartment) = "MG" Or UCase(vDepartment) = "IT" Or UCase(vDepartment) = "HR" Or UCase(vDepartment) = "AV" Then
               Call PrintRequestQuotation
            Else
            MsgBox "รหัสผู้ใช้งานไม่มีสิทธิ์พิมพ์เอกสารดังกล่าว กรุณาตรวจสอบ", vbCritical, "Send Error Message"
            Exit Sub
            End If
...
If KeyAscii = 13 Then
    ...
    If vDocTypeID = "QT" Then
      ...
      Me.CHKQuotation.Visible = True
      Call GetQuotationCondition
    Else
      Me.ListViewCondition.Visible = False
      Me.LBLQuotation.Visible = False
      Me.CHKQuotation.Visible = False
    End If
...
On Error GoTo ErrDescription
If UCase(Left(TXT011.Text, 3)) = "S01" Or UCase(Left(TXT011.Text, 3)) = "S02" Or UCase(Left(TXT011.Text, 3)) = "W01" Or UCase(Left(TXT011.Text, 3)) = "W02" Then
...
If vDocGroup = "BOA" Or vDocGroup = "BOT" Or vDocGroup = "BBT" Or vDocGroup = "BBL" Then
    vTable = "DBO.BCCHQINDEPOSIT"
    vTypeDoc = "BD"
    Me.ListViewCondition.Visible = False
    Me.LBLQuotation.Visible = False
End If

If UCase(Left(Right(vDocGroup, Len(vDocGroup) - InStr(vDocGroup, "-")), 2)) = "IF" Then 'Left(vDocGroup, 2)
    vTable = "DBO.BCSTKTRANSFER2"
    vTypeDoc = "TF"
    Me.ListViewCondition.Visible = False
    Me.LBLQuotation.Visible = False
End If

If UCase(Left(Right(vDocGroup, Len(vDocGroup) - InStr(vDocGroup, "-")), 2)) = "PR" Then  'Left(vDocGroup, 2) = "PR" Then
    vTable = "DBO.BCSTKRequest"
    vTypeDoc = "RQ"
    Me.ListViewCondition.Visible = False
    Me.LBLQuotation.Visible = False
End If
...
If vCheckDocNo = 0 Then 'เช็คว่าพิมพ์ครั้งแรกหรือ ทดแทน
    vQuery = "select docno,billtype from dbo.BCQuotation where docno = '" & vDocNo & "' "
    If OpenDataBase(gConnection, vRecordset, vQuery) <> 0 Then
...
End If 'เช็คว่าพิมพ์ครั้งแรกหรือ ทดแทน
        ...
        If Me.CHKQuotation.Value = 0 Then
           vRepType = "QT"
...
'vQuery = "select reportname from bcreportname where repid = '" & vRepID & "'  and reptype = '" & vRepType & "' "
If OpenDataBase(gConnection, vRecordset, vQuery) <> 0 Then
    With CrystalQuotation 'CrystalReport1
        .ReportFileName = Trim(vRecordset.Fields("reportname").Value) & ".rpt"
        .ParameterFields(0) = "@DocNo;" & vDocNo & ";true"
        .Destination = crptToWindow
        .WindowState = crptMaximized
...
If UCase(vGroupDoc) = "SHV" Or UCase(vGroupDoc) = "SHN" Or UCase(vGroupDoc) = "SCV" Or UCase(vGroupDoc) = "SCN" Or UCase(vGroupDoc) = "SAB" Or UCase(vGroupDoc) = "SVE" Then  'Or UCase(vGroupDoc) = "SVD" Or UCase(vGroupDoc) = "SVN" Or UCase(vGroupDoc) = "SVM" Then
        ...
        If vCheckDocNo = 0 Then 'เช็คว่าพิมพ์ครั้งแรกหรือ ทดแทน
            vQuery = "select docno,billtype from dbo.bcsaleorder where docno = '" & vDocNo & "' and sostatus = 0 "
            If OpenDataBase(gConnection, vRecordset, vQuery) <> 0 Then
                vCheckBillType = Trim(vRecordset.Fields("billtype").Value)
            End If
            vRecordset.Close
...
vDocNo = Trim(TXT011.Text)
vQuery = "select typecode from dbo.vw_IV_PackingSlip where docno = '" & vDocNo & "' and shelfgroup1 = 'M' "
If OpenDataBase(gConnection, vRecordset, vQuery) <> 0 Then
    vCheckValue = True
Else
    vCheckValue = False
...
vDocNo = Trim(TXT011.Text)
vQuery = "select itemcode from dbo.bcsaleordersub  where docno = '" & vDocNo & "' and whcode = '014' and  typecode not in (select itemtype from npmaster.dbo.NP_ItemOutLet) "
If OpenDataBase(gConnection, vRecordset, vQuery) <> 0 Then
    vCheckValue1 = True
Else
    vCheckValue1 = False
...
If UCase(vGroupDoc) = "ROV" Or UCase(vGroupDoc) = "RON" Or UCase(vGroupDoc) = "RWV" Or UCase(vGroupDoc) = "RWN" Then
        ...
        If vCheckDocNo = 0 Then 'เช็คว่าพิมพ์ครั้งแรกหรือ ทดแทน
            vQuery = "select docno,billtype from dbo.bcsaleorder where docno = '" & vDocNo & "' and sostatus =1"
            If OpenDataBase(gConnection, vRecordset, vQuery) <> 0 Then
                vCheckBillType = Trim(vRecordset.Fields("billtype").Value)
            End If
            vRecordset.Close
        ...
        vQuery = "select sostatus from dbo.bcsaleorder where docno = '" & vDocNo & "' "
        If OpenDataBase(gConnection, vRecordset, vQuery) <> 0 Then
...
'On Error GoTo ErrDescription
    ...
    vDocNo = UCase(Trim(TXT011.Text))
    vCheckHeader = UCase(Left(Right(vDocNo, Len(vDocNo) - InStr(vDocNo, "-")), 3))  'Left(vDocNo, 3)
    vQuery = "select docno,runnumber  from npmaster.dbo.TB_DC_RunNumberDocumentLogs where docno = '" & vDocNo & "' "
    If OpenDataBase(gConnection, vRecordset, vQuery) <> 0 Then
        vCheckDocNo = 1 'เคยพิมพ์แล้ว
        vRunNumber = Trim(vRecordset.Fields("runnumber").Value)
...
Public Sub PrintStockRequestApprove()
Dim vQuery As String
Dim vRecordset As New ADODB.Recordset
Dim vDocNo As String
Dim vGroupDoc As String
...
'On Error GoTo ErrDescription
    ...
    vDocNo = UCase(Trim(TXT011.Text))
    vCheckHeader = Left(Right(vDocNo, Len(vDocNo) - InStr(vDocNo, "-")), 3) 'Left(vDocNo, 3)
    vQuery = "select docno,runnumber  from npmaster.dbo.TB_DC_RunNumberDocumentLogs where docno = '" & vDocNo & "' "
    If OpenDataBase(gConnection, vRecordset, vQuery) <> 0 Then
        vCheckDocNo = 1 'เคยพิมพ์แล้ว
        vRunNumber = Trim(vRecordset.Fields("runnumber").Value)
...
End If 'เช็คว่าพิมพ์ครั้งแรกหรือ ทดแทน
        ...
        If vCheckHeader = "PRE" Then
         MsgBox "ไม่สามารถพิมพ์เอกสารได้ เนื่องจากข้อมูลไม่ตรงกับฟอร์มที่จะพิมพ์", vbCritical, "Send Error Message"
         Exit Sub
        Else
         vRepID = 420
        End If
...
Public Sub PrintRequestQuotation()
Dim vQuery As String
Dim vRecordset As New ADODB.Recordset
Dim vDocNo As String
Dim vGroupDoc As String
...
End If 'เช็คว่าพิมพ์ครั้งแรกหรือ ทดแทน
        ...
        vVendorName = InputBox("กรณี ต้องการระบุชื่อของเจ้าหนี้ให้กรอกชื่อเจ้าหนี้ในช่องข้างล่าง", "กรอกชื่อเจ้าหนี้ เพื่อพิมพ์ใบขอเสนอราคาสินค้า", "")
        ...
        If OpenDataBase(gConnection, vRecordset, vQuery) <> 0 Then
            With Crystal001
                ...
                .Formulas(0) = "computername='" & vComputerName1 & "' "
                .Formulas(1) = "username='" & vUserName1 & "' "
                .Formulas(2) = "vendorname='" & vVendorName & "' "
                .Action = 1
            End With
        End If
        vRecordset.Close
